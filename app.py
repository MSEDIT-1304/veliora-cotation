...
# ✅ AJOUT : NOMBRE DE PLACES (inclut 7)
...
if st.button("Calculer l'estimation"):
    ...
    # ✅ PRIORITÉ AU % SI UTILISÉ
    if commission_pct > 0:
        commission_calc = prix * (commission_pct / 100)
    else:
        commission_calc = commission

    net = int(prix - commission_calc)

    st.markdown(f"### 💰 Prix estimé : {prix} €")
    st.markdown(f"### 🧾 Commission : {int(commission_calc)} €")
